...
def collect_reviews(app_id, country, app_name=None):
    """
    ...
    Args:
        ...
        app_name: 앱 이름 (사용되지 않음, 호환성을 위해 유지)
        max_pages: 최대 페이지 수 (기본값 10, 각 페이지당 최대 50개)
    """
    ...
    try:
        for page in range(1, max_pages + 1):
            ...
            print(f"[{country.upper()}] 페이지 {page} 수집 중...", end=' ')
            ...
            for entry in entries:
                # 앱 정보 항목은 건너뛰기 (im:name이 있으면 리뷰가 아님)
                if 'im:name' in entry:
                    continue
        ...
        print(f"[{country.upper()}] 총 {len(all_reviews)}개의 리뷰 수집 완료")
        return all_reviews
...
def analyze_reviews(reviews, country):
    ...
    # 날짜 변환 (timezone 정보를 제거하여 엑셀 호환성 확보)
...
def basic_statistics(df, country):
    ...
    if 'date' in df.columns:
        print(f"\n리뷰 기간:")
        print(f"  최초: {df['date'].min()}")
        print(f"  최신: {df['date'].max()}")
...
def main():
    ...
    # 미국 리뷰 수집
    reviews_us = collect_reviews(app_id, 'us')
    if reviews_us:
    ...
    # 한국 리뷰 수집
    reviews_kr = collect_reviews(app_id, 'kr')
    if reviews_kr:
